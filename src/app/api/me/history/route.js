...
export async function GET(request) {
  ...
  const { data, error } = await supabase
    .from("watch_history")
    .select("last_watched_at,video:videos!watch_history_video_id_fkey(id,user_id,title,duration_sec,views_count,created_at,thumbnail_path,channel:profiles!videos_user_id_fkey(username,display_name,avatar_url))")
    .eq("user_id", user.id)
  ...
  const items = await Promise.all(
    (data || []).map(async (row) => {
      ...
      return { ...video, thumbnail_url, watched_at: row.last_watched_at };
    })
  );
