...
export async function GET(request) {
  ...
  const { data, error } = await supabase
    ...
    .select("reaction,updated_at,video:videos!video_reactions_video_id_fkey(id,title,duration_sec,views_count,created_at,thumbnail_path,channel:profiles!videos_user_id_fkey(username,display_name,avatar_url))")
    .eq("user_id", user.id)
